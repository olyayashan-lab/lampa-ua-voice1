...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  ...
  // UA Voice: жорсткий український режим.
  // Не додає нових стрімінгових сайтів. Працює тільки з тим,
  // що вже повертає цей плагін.
  function uaNormalize(value) {
    return (value || '').toString().toLowerCase().replace(/ё/g, 'е').trim();
  }

  function isUkrainianVoice(value) {
    var s = uaNormalize(value);

    if (
      s.indexOf('україн') !== -1 ||
      s.indexOf('украин') !== -1 ||
      s.indexOf('ukrain') !== -1 ||
      /(^|[\s\[\(\-_|])ukr([\s\]\)\-_|]|$)/i.test(s) ||
      /(^|[\s\[\(\-_|])uk([\s\]\)\-_|]|$)/i.test(s) ||
      /(^|[\s\[\(\-_|])ua([\s\]\)\-_|]|$)/i.test(s)
    ) return true;
  ...
  var uaAllowedBalansers = [
    'kinoukr',
    'uafilm',
    'uakino'
  ];

  function isAllowedUaBalanser(name) {
    name = uaNormalize(name);
    for (var i = 0; i < uaAllowedBalansers.length; i++) {
      if (name === uaAllowedBalansers[i] || name.indexOf(uaAllowedBalansers[i]) === 0) return true;
    }
    return false;
  }
  ...
  function component(object) {
    ...
    this.startSource = function(json) {
      return new Promise(function(resolve, reject) {
        json.forEach(function(j) {
          ...
          if (!isAllowedUaBalanser(name)) return;
          ...
          if (!red) {
            ...
            if (_filter.length) {
              ...
              resolve(json.online.filter(function(c) {
                return c.show && isAllowedUaBalanser(balanserName(c));
              }));
            } else if (any) {
              reject();
            }
    ...
    this.parse = function(str) {
      ...
      try {
        ...
        var buttons_all = this.parseJsonDate(str, '.videos__button');
        var buttons = buttons_all.filter(function(b) {
          return isUkrainianVoice(b.text || b.title || b.voice_name || '');
        });
        if (items.length == 1 && items[0].method == 'link' && !items[0].similar) {
          ...
          if (videos.length) {
            if (buttons_all.length && !buttons.length) {
              this.activity.loader(false);
              this.empty();
              return;
            }
  ...
  function startPlugin() {
    ...
    var manifst = {
      ...
      version: '1.7.1-UA2',
      name: 'UA Voice 🇺🇦',
      description: 'Український режим: лише українські джерела та озвучки',
      component: 'bwarch',
